...
2. このハンズオンで使用している App Service、`MovieApp-[チームID]` を選択し、画面左のメニューから \[**デプロイ スロット**\] を選択します
...
1. [Azure ポータル](https://portal.azure.com/)にログインし、演習で使用している App Service、`MovieApp-[チームID]` を選択します
...
2. \[**モニター**\] 画面に遷移するので画面左のメニューから \[**自動スケーリング**\] を選択します。
    ...
    自動スケーリングが適用できるすべてのリソースが表示されるので、演習で使用している App Service、`MovieApp-[チームID]` の App Service プランの名前を選択します
...
5. \[**自動スケーリング設定**\] の画面に戻るので画面下部の \[**インスタンスの制限**\] で \[**最小値 \***\] の値を `1`、\[**最大値 \***\] の値を `3`、\[**規定値 \***\] の値を `1` に変更します。
...
Azure Load Testing を使用して負荷をかける対象となる Web アプリケーション MovieApp-[チームID] には [演習 2 のタスク 6](ex02.md#%E3%82%BF%E3%82%B9%E3%82%AF-6--%E8%AA%8D%E8%A8%BC%E3%81%AB%E3%81%A4%E3%81%84%E3%81%A6) で App Service の自動認証が設定されており、認証されていないユーザーはアプリケーションの画面にアクセスできないようになっているためこのままでは Azure Load Testing の[クイックテスト](https://learn.microsoft.com/ja-jp/azure/load-testing/quickstart-create-and-run-load-test?tabs=virtual-users#create-a-load-test)で負荷をかけることができません。

次の手順を実施する前に App Service の自動認証を無効にします。

具体的な手順は以下のとおりです。

1. [Azure ポータル](https://portal.azure.com/)で、演習で使用している App Service、`MovieApp-[チームID]` を選択します
...
Azure Load Testing を使用して負荷をかける対象となる Web アプリケーション MovieApp-[チームID] はプライベートなAppServiceであり、ネットワークの構成が必要となります。

具体的な手順は以下の通りです。

1. [Azure ポータル](https://portal.azure.com/)で、演習で使用している 仮想ネットワーク、`handson-vnet-AG` を選択します

2. 画面左のメニューから \[**サブネット**\] を選択し、\[**＋サブネット**\]を選択します

    <img src="images/20240721_ex03_task2_002.jpg">

3. \[**サブネットの追加**\] 画面で、各項目を以下の通り入力します

    |項目|値|
    |---|---|
    |サブネットの目的|デフォルト|
    |名前|`handson-subnet-LoadTesting-[チームID]`|
    |IPv4アドレス空間を含める|☑選択|
    |Ipv4アドレスの範囲 |10.1.0.0/16  |
    |開始アドレス|10.1.3.0|
    |サイズ|/24(256のアドレス)|

    <img src="images/20240721_ex03_task2_003.jpg">

    入力が完了したら、\[**追加**\]をクリックします。

ここまでの手順で LoadTestingのプライベートに必要なサブネットの追加が完了しました。
...
6. \[**URL ベースのテストを作成する**\] の画面に遷移するので、
上タブ\[**テスト計画**\] をクリックします<br>
項目 \[**＋要求の追加**\]をクリックし \[**URL\***\]に演習用 Web アプリケーションの URL を入力します

    <img src="images/20240721_ex03_task2_005.jpg" width="700px">
    <img src="images/20240721_ex03_task2_006.jpg" width="700px">

1. 上タブ\[**読み込み**\] をクリックします<br>
\[**テスト期間 (分) \***\] と \[**増加時間 (分) \***\]  を以下のように設定し、画面下部の\[**確認及び作成**\]をクリックし\[**作成**\]をクリックします
    ...
    **ネットワーク**
    ...
    | 項目 | 設定値  |
    | ----------- | ------- |
    | テスト トラフィック モードの構成 \*| **非公開** |
    | 仮想ネットワーク \*| **handson-vnet-AG** |
    | サブネット \*| `handson-subnet-LoadTesting-[チームID]` |
...
1. スケールアウトが実行されたか確認するために自動スケールのログを確認します
...
2.  自動スケーリングが適用できるすべてのリソースが表示されるので、演習で使用している App Service、`MovieApp-[チームID]` の App Service プランの名前を選択します

    <img src="images/AppService_AutoScaling_Choseplan.png" width="700px">

3.  \[**自動スケーリング設定**\] の画面に遷移するので、画面上部の \[**実行履歴**\] タブをクリックして選択します
...
<!--
なお、この演習では [**タスク 1.2 : GitHub リポジトリを使用した CI/CD**]
(#%E3%82%BF%E3%82%B9%E3%82%AF-12--github-%E3%83%AA%E3%83%9D%E3%82%B8%E3%83%88%E3%83%AA%E3%82%92%E4%BD%BF%E7%94%A8%E3%81%97%E3%81%9F-cicd) の演習でスワップしたステージング スロット **MovieApp-XYZ/staging** を使用します。これは、このスロットに修正したプロジェクトをデプロイする前の、HTTP500 エラーを発生させるコード含むプロジェクトがスワップされて残っているためです。
-->

具体的な手順は以下のとおりです。

1. Azure ポータルで App Servive **MovieApp-[チームID]** を選択し、\[**概要**\] 画面の \[**既定の URL**\] を仮想マシン上で検索し、アプリケーションの画面が正常に表示されるのを確認します
...
10. アラートルールの作成が完了したら、App Servive **MovieApp-[チームID]** の以下の URL にアクセスして SPA 画面を表示します
...
3. \[**Log Analytics ワークスペースの作成**\] 画面に遷移し \[**基本**\] タブが表示されるので、同タブの各項目を以下のように設定します
    ...
    **インスタンスの詳細**
    ...
    |項目|値|
    |---|---|
    |名前 \*| `PaaSHandson-LogAnalytics-[チームID]` |
    |地域 \*| \[**Japan East**\] |
...
4. デプロイが完了したら、Azure ポータルの[ホーム画面](https://portal.azure.com/#home) から App Service `MovieApp-[チームID]` アイコンをクリックし、画面左のメニューの \[**監視**\] グループにある \[**診断設定**\] をクリックします
...
6. 保存が完了したら、Azure ポータルの[ホーム画面](https://portal.azure.com/#home) から App Service `MovieApp-[チームID]` アイコンをクリックし、画面左のメニューの \[**監視**\] グループにある \[**ログ**\] をクリックします
    ...
    もし表示されていない場合は、画面上部の \[**スコープの選択**\] ボタンをクリックし、表示された Azure リソースの一覧から `MovieApp-[チームID]` を選択し \[**適用**\] ボタンをクリックします。
...
9. HTTP500 (HTTP Server Error) をフィルターするクエリーを保存します
...
10. クエリーエディタの画面に戻るので \[**クエリ**\] タブをアクティブにし、検索ボックスに `HTTPServerError` と入力してキーボードの \[Enter\] キーを押下ます
...
4. Application Insights で取得された情報を確認します。
    ...
    Azure ポータルの[ホーム画面](https://portal.azure.com/#home)からリソースグループ [**handson-RG-[チームID]**\] を開き、`MovieApp-[チームID]` という Application Insights が作成されていることを確認し、同アイコンをクリックします。

5. \[**概要**\] 画面が表示されるので、同画面のグラフ \[**失敗した要求**\]、\[**サーバー要求**\] にリクエストが検出されていることを確認します

    <img src="images/AppInsights_Datail_Graph.png" width="700px">

6. 画面左の \[**アプリケーション マップ**\] をクリックすると、クライアント、SQL データベース、アプリケーションの図が表示されるので、`MovieApp-[チームID]` をクリックするか、マウスをホバーした際に表示さるメニューから \[**詳細を表示**\] をクリックします。
